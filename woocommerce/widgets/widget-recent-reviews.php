	 ...
	 public function widget( $args, $instance ) {
		...
		if ( $comments ) {
			...
			foreach ( (array) $comments as $comment ) {
				...
				echo '<div class="media widget-product"><a href="' . esc_url( get_comment_link( $comment->comment_ID ) ) . '" class="image pull-left">';

				echo $_product->get_image().'</a>';

				echo '<div class="media-body"><h3 class="name"><a href="' . esc_url( get_comment_link( $comment->comment_ID ) ) . '">';

				echo $_product->get_title().'</a></h3>';

				echo '<div class="rating pull-left">';
					echo $rating_html;
				echo '</div>';

				printf( '<div class="review"><span class="reviewer">' . _x( 'by %1$s', 'by comment author', 'woocommerce' ) . '</span></div>', get_comment_author() );
